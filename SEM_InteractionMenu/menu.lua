	...
	while true do
		...
		if IsControlJustPressed(1, Config.MenuButton) and GetLastInputMethod(2) then
			if not menuOpen then
                print('[SEM_InteractionMenu] Opening menu...')
                
                GetDutyStatus(function(leoDuty, fireDuty)
                    print('[SEM_InteractionMenu] Building menu with duty status: LEO=' .. tostring(leoDuty) .. ', Fire=' .. tostring(fireDuty))
                    
                    if Config.LEOAccess == 5 then
                        print('[SEM_InteractionMenu] LEO Access check: Ace=' .. tostring(LEOAce) .. ', Duty=' .. tostring(leoDuty))
                    end
                    if Config.FireAccess == 5 then
                        print('[SEM_InteractionMenu] Fire Access check: Ace=' .. tostring(FireAce) .. ', Duty=' .. tostring(fireDuty))
                    end
