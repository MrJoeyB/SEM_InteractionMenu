...
AddEventHandler('SEM_InteractionMenu:LEOPermsResult', function(Allowed)
    if Allowed then
        LEOAce = true
        print('[SEM_InteractionMenu] LEO Ace Permission: GRANTED')
    else
        LEOAce = false
        print('[SEM_InteractionMenu] LEO Ace Permission: DENIED')
    end
end)
...
AddEventHandler('SEM_InteractionMenu:FirePermsResult', function(Allowed)
    if Allowed then
        FireAce = true
        print('[SEM_InteractionMenu] Fire Ace Permission: GRANTED')
    else
        FireAce = false
        print('[SEM_InteractionMenu] Fire Ace Permission: DENIED')
    end
end)
...
local IsFireOnDuty = false
local DutyStatusReceived = false

RegisterNetEvent('SEM_InteractionMenu:ReceiveDutyStatus')
AddEventHandler('SEM_InteractionMenu:ReceiveDutyStatus', function(leoDuty, fireDuty)
    IsLEOOnDuty = leoDuty
    IsFireOnDuty = fireDuty
    DutyStatusReceived = true
    print('[SEM_InteractionMenu] Duty Status Received: LEO=' .. tostring(leoDuty) .. ', Fire=' .. tostring(fireDuty))
end)

function GetDutyStatus(callback)
    DutyStatusReceived = false
    TriggerServerEvent('SEM_InteractionMenu:GetDutyStatus')
    
    local timeout = 0
    while not DutyStatusReceived and timeout < 100 do
        Citizen.Wait(10)
        timeout = timeout + 1
    end
    
    if callback then
        callback(IsLEOOnDuty, IsFireOnDuty)
    end
end

function IsOnLEODuty()
...
function LEORestrict()
    ...
    elseif Config.LEOAccess == 4 then
        return LEOAce and IsPedInAnyPoliceVehicle(PlayerPedId())
    elseif Config.LEOAccess == 5 then
        return LEOAce and IsLEOOnDuty
    end
end
...
function FireRestrict()
    ...
    elseif Config.FireAccess == 1 then
        return IsPedInAnyVehicle(PlayerPedId()) and (GetVehicleClass(GetVehiclePedIsIn(PlayerPedId())) == 18 or GetVehicleClass(GetVehiclePedIsIn(PlayerPedId())) == 15)
    elseif Config.FireAccess == 2 then
        return FireAce
    elseif Config.FireAccess == 3 then
        return FireOnduty
    elseif Config.FireAccess == 4 then
        return FireAce and IsPedInAnyVehicle(PlayerPedId()) and (GetVehicleClass(GetVehiclePedIsIn(PlayerPedId())) == 18 or GetVehicleClass(GetVehiclePedIsIn(PlayerPedId())) == 15)
    elseif Config.FireAccess == 5 then
        return FireAce and IsFireOnDuty
    end
end

function LEOMenuRestrict()
    if Config.LEOAccess == 5 then
        local result = LEOAce and IsLEOOnDuty
        return result
    else
        return LEORestrict()
    end
...
function FireMenuRestrict()
    if Config.FireAccess == 5 then
        local result = FireAce and IsFireOnDuty
        return result
    else
        return FireRestrict()
    end
