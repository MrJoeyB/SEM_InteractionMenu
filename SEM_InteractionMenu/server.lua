...
RegisterServerEvent('SEM_InteractionMenu:CheckLEODuty')
AddEventHandler('SEM_InteractionMenu:CheckLEODuty', function()
    local src = source
    local isOnDuty = false
    
    if Config.LEOAccess == 5 then
        if GetResourceState('Lumen-Duty') == 'started' then
            isOnDuty = exports['Lumen-Duty']:IsPlayerOnDuty(src)
            print('[SEM_InteractionMenu] Checking LEO Duty for player ' .. GetPlayerName(src) .. ' (ID: ' .. src .. '): ' .. tostring(isOnDuty))
        else
            print('[SEM_InteractionMenu] ERROR: Lumen-Duty resource not found or not started')
        end
    end
    
    TriggerClientEvent('SEM_InteractionMenu:LEODutyResult', src, isOnDuty)
end)

RegisterServerEvent('SEM_InteractionMenu:CheckFireDuty')
AddEventHandler('SEM_InteractionMenu:CheckFireDuty', function()
    local src = source
    local isOnDuty = false
    
    if Config.FireAccess == 5 then
        if GetResourceState('Lumen-Duty') == 'started' then
            isOnDuty = exports['Lumen-Duty']:IsPlayerOnDuty(src)
            print('[SEM_InteractionMenu] Checking Fire Duty for player ' .. GetPlayerName(src) .. ' (ID: ' .. src .. '): ' .. tostring(isOnDuty))
        else
            print('[SEM_InteractionMenu] ERROR: Lumen-Duty resource not found or not started')
        end
    end
    
    TriggerClientEvent('SEM_InteractionMenu:FireDutyResult', src, isOnDuty)
end)

RegisterServerEvent('SEM_InteractionMenu:GetDutyStatus')
AddEventHandler('SEM_InteractionMenu:GetDutyStatus', function()
    local src = source
    local leoDuty = false
    local fireDuty = false
    
    if Config.LEOAccess == 5 then
        if GetResourceState('Lumen-Duty') == 'started' then
            leoDuty = exports['Lumen-Duty']:IsPlayerOnDuty(src)
        end
    end
    
    if Config.FireAccess == 5 then
        if GetResourceState('Lumen-Duty') == 'started' then
            fireDuty = exports['Lumen-Duty']:IsPlayerOnDuty(src)
        end
    end
    
    TriggerClientEvent('SEM_InteractionMenu:ReceiveDutyStatus', src, leoDuty, fireDuty)
end)

RegisterServerEvent('SEM_InteractionMenu:LEOPerms')
AddEventHandler('SEM_InteractionMenu:LEOPerms', function()
    local src = source
    if IsPlayerAceAllowed(src, 'sem_intmenu.leo') then
		TriggerClientEvent('SEM_InteractionMenu:LEOPermsResult', src, true)
	else
		TriggerClientEvent('SEM_InteractionMenu:LEOPermsResult', src, false)
	end
end)
...
AddEventHandler('SEM_InteractionMenu:FirePerms', function()
    local src = source
    if IsPlayerAceAllowed(src, 'sem_intmenu.fire') then
		TriggerClientEvent('SEM_InteractionMenu:FirePermsResult', src, true)
	else
		TriggerClientEvent('SEM_InteractionMenu:FirePermsResult', src, false)
	end
end)
...
AddEventHandler('SEM_InteractionMenu:UnjailPerms', function()
    local src = source
    if IsPlayerAceAllowed(src, 'sem_intmenu.unjail') then
		TriggerClientEvent('SEM_InteractionMenu:UnjailPermsResult', src, true)
	else
		TriggerClientEvent('SEM_InteractionMenu:UnjailPermsResult', src, false)
	end
end)
...
AddEventHandler('SEM_InteractionMenu:UnhospitalPerms', function()
    local src = source
    if IsPlayerAceAllowed(src, 'sem_intmenu.unhospital') then
		TriggerClientEvent('SEM_InteractionMenu:UnhospitalPermsResult', src, true)
	else
		TriggerClientEvent('SEM_InteractionMenu:UnhospitalPermsResult', src, false)
	end
end)
